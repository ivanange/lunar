<div>
    ...
    <x-hub::slideover :title="__('adminhub::components.brand-search.title')" wire:model="showBrowser">
        <div class="space-y-4" x-data="{ tab: 'search' }">
            <div>
                <nav class="flex space-x-4" aria-label="Tabs">
                    <button x-on:click.prevent="tab = 'search'" class="px-3 py-2 text-sm font-medium rounded-md" :class="{
                                'bg-primary-100 text-primary-700': tab == 'search',
                                'text-gray-500 hover:text-gray-700': tab != 'search'
                            }">
                        {{ __('adminhub::components.brand-search.first_tab') }}
                    </button>

                    <button class="px-3 py-2 text-sm font-medium rounded-md" @click.prevent="tab = 'selected'" :class="{
                                'bg-primary-100 text-primary-700': tab == 'selected',
            ...
            <div x-show="tab == 'search'">
                ...
                <span class="block p-3 my-2 text-xs text-primary-600 rounded bg-primary-50">
                ...
                <div class="mt-4 space-y-1">
                    ...
                    <div @class([ 'flex w-full items-center justify-between rounded shadow-sm text-left border px-2 py-2 text-sm'
                        ...
                        @else
                        <button
                            class="px-2 py-1 text-xs text-primary-700 border border-primary-200 rounded shadow-sm hover:bg-primary-50"
