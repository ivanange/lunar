<div>
    ...
    <div class="pb-24 mt-8 lg:gap-8 lg:flex lg:items-start">
        <div class="space-y-6 lg:flex-1">
            ...
            <div class="shadow sm:rounded-md" id="products">
                <div class="flex-col px-4 py-5 space-y-4 bg-white rounded sm:p-6">
                    ...
                    <div wire:sort sort.options='{ group: "products", method: "sortProducts" }' class="space-y-2">
                        ...
                        <div sort.item="products" sort.id="{{ $product['id'] }}" wire:key="product_{{ $product['id'] }}"
                        ...
                        <div wire:loading.remove wire:target="loadProducts">
                            <x-hub::button theme="gray" type="button" wire:click="loadProducts">Load {{
                                $this->productCount }} products</x-hub::button>
                        </div>
            ...
            <x-hub::modal.dialog wire:model="showCreateChildForm" form="createChildCollection">
                <x-slot name="title">
                ...
                <x-slot name="content">
                    <div class="space-y-4">
                        <x-hub::input.group :label="__('adminhub::inputs.name')" for="name"
                            :error="$errors->first('childCollection.name')" required>
                            <x-hub::input.text wire:model="childCollection.name"
                                :error="$errors->first('childCollection.name')" />
                        </x-hub::input.group>

                        @if ($this->slugIsRequired)
                        <x-hub::input.group :label="__('adminhub::inputs.slug.label')" for="slug"
                            :error="$errors->first('slug')" required>
                ...
                <x-slot name="footer">
                    <x-hub::button type="button" wire:click.prevent="$set('showCreateChildForm', false)" theme="gray">
            ...
            <div class="shadow sm:rounded-md" id="collections">
                <div class="flex-col px-4 py-5 space-y-4 bg-white rounded sm:p-6">
                    ...
                    <div class="space-y-2" wire:sort sort.options='{group: "collection_child", method: "sort"}'>
                        @forelse($children as $child)
                        <div wire:key="child_{{ $child['id'] }}" sort.item="collection_child"
                            sort.id="{{ $child['id'] }}" class="flex items-center">
            ...
            <x-hub::modal.dialog wire:model="showDeleteConfirm">
                ...
                <x-slot name="footer">
                    <div class="flex items-center justify-end space-x-4">
                        ...
                        <x-hub::button wire:click="deleteCollection" theme="danger">
        ...
        <x-hub::layout.page-menu>
            <nav class="space-y-2" aria-label="Sidebar" x-data="{ activeAnchorLink: '' }"
                ...
                <a href="#{{ $item['id'] }}" @class([ 'flex items-center gap-2 p-2 rounded text-gray-500'
                    ...
                    :class="{
                    'bg-primary-50 text-primary-700 hover:text-primary-600': linkId === activeAnchorLink
                    }"
