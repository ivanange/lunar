<div>
  ...
  <div class="space-y-4">
    ...
    <div class="overflow-hidden border rounded shadow-sm">
      <header
        class="flex items-center justify-between px-4 py-2 text-xs font-medium text-gray-500 uppercase border-b bg-gray-50">
        {{ $option->translate('name') }}
        <div class="flex items-center space-x-4">
          <button wire:click.prevent="$set('selectedOption', '{{ $option->id }}')"
          ...
          <button type="button" wire:click.prevent="toggle('{{ $option->id }}')"
      ...
      <div class="p-4 space-y-2 bg-white">
        <div class="flex flex-wrap items-center gap-2">
          ...
          <label class="relative block" wire:key="option_{{ $value->id }}">
            <input type="checkbox" class="absolute mt-2 left-2 peer" wire:model.debounce.100ms="selectedValues"
              value="{{ $value->id }}">
            <span
              class="items-center inline-block px-2 py-1 pl-6 space-x-1 text-sm border rounded shadow-sm cursor-pointer hover:bg-gray-100 bg-gray-50 peer-checked:bg-primary-200 peer-checked:text-primary-900 peer-checked:border-primary-500">{{
              $value->translate('name') }}</span>
          </label>
          @endforeach
        </div>
      </div>
