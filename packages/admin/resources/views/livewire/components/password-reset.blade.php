<div>
    ...
    <div class="mt-8 space-y-4">
        ...
        <p>
            <a href="{{ route('hub.login') }}" class="text-primary-600 transition hover:text-primary-500">
                {{ __('adminhub::auth.reset-password.back_link') }}
            </a>
        </p>
    </div>
    @else
    <form method="POST" wire:submit.prevent="process" class="mt-8 space-y-6">
        @csrf
        ...
        <div>
            ...
            <div class="relative">
                <span @class([ 'absolute inset-y-0 grid text-primary-600 left-3 place-content-center'
                ...
                <input id="password" name="password" type="password" autocomplete="new-password"
                    wire:model.defer="password" placeholder="{{ __('adminhub::inputs.new_password') }}"
                    @class([ 'w-full py-3 pl-10 pr-3 text-gray-900 dark:text-white rounded-md shadow-sm dark:bg-gray-800 sm:text-sm form-input'
        ...
        <div>
            <button type="submit"
                class="w-full p-3 text-sm text-white transition bg-primary-600 rounded-md hover:bg-primary-500">
                <div wire:loading.delay wire:target="process">
                    <x-hub::loading-indicator class="w-5 h-5 mx-auto" />
                </div>

                <div wire:loading.delay.remove wire:target="process">
                    <span>
