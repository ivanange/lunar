                ...
                <x-hub::modal.dialog wire:model="showImageSelectModal">
                    ...
                    <x-slot name="content">
                        <div class="grid grid-cols-4 gap-4 overflow-y-auto max-h-96">
                            ...
                            <label @class([ 'cursor-pointer'=> !in_array($productImage->id, $this->currentImageIds),
                                ...
                                <input wire:model="selectedImages" name="selectedImages" value="{{ $productImage->id }}"
                                    class="sr-only peer" type="checkbox" @if (in_array($productImage->id,
                                $this->currentImageIds)) disabled @endif>
                                <img src="{{ $productImage->getFullUrl('small') }}"
                                    class="border-2 border-transparent rounded-lg shadow-sm peer-checked:border-primary-500">
                            </label>
                            @empty
                ...
                <div class="flex items-center justify-between p-4 bg-white border rounded-md shadow-sm"
                    ...
                    <div class="flex items-center w-full space-x-6">
                        ...
                        <div class="flex flex-shrink-0" x-data="{ imageBlob: null }">
                            <button type="button" wire:click="$set('images.{{ $key }}.preview', true)">
                        ...
                        <div class="w-full">
                            <x-hub::input.text wire:model="images.{{ $key }}.caption" placeholder="Enter Alt. text" />
                        </div>
                        ...
                        <div class="flex items-center ml-4 space-x-4">
                            ...
                            <button type="button" wire:click="$set('images.{{ $key }}.edit', true)">
                                <x-hub::icon ref="pencil" style="solid" class="text-gray-400 hover:text-primary-500" />
                            </button>

                            <x-hub::tooltip
                                :text="$image['primary'] ? __('adminhub::partials.image-manager.delete_primary') : __('adminhub::partials.image-manager.delete_row_btn')">
                                <button type="button" wire:click.prevent="removeImage('{{ $key }}')"
                                    class="text-gray-400 hover:text-red-500 disabled:opacity-50 disabled:cursor-not-allowed"
                                    @if($image['primary']) disabled @endif>
                                    <x-hub::icon ref="trash" style="solid" class="text-gray-400 hover:text-red-500" />
                                </button>
                            </x-hub::tooltip>
                        </div>
                    </div>
