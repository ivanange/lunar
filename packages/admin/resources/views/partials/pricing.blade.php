        ...
        <div class="flex-col px-4 py-5 space-y-4 bg-white sm:p-6">
                ...
                <div class="flex items-center space-x-2">
                    <div>
                        <select wire:change="setCurrency($event.target.value)"
                            class="block w-full py-1 pl-2 pr-8 text-base text-gray-600 bg-gray-100 border-none rounded-md form-select focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
                            @foreach($this->currencies as $c)
                            <option value="{{ $c->id }}" @if($currency->id == $c->id) selected @endif>{{ $c->code }}
                            </option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="grid grid-cols-2 gap-4 hidden">
                <div>
                    <x-hub::input.group :label="__('adminhub::inputs.tax_class.label')" for="tax_class">
                        <x-hub::input.select id="tax_class" wire:model="variant.tax_class_id">
                            ...
                            <option wire:key="tax_class_{{ $taxClass->id }}" value="{{ $taxClass->id }}">{{
                ...
                <div>
                    <x-hub::input.group :label="__('adminhub::inputs.tax_ref.label')"
                        ...
                        :errors="$errors->get('variant.tax_ref')" for="unit_quantity">
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-3 gap-4">
                    <x-hub::input.group :label="__('adminhub::inputs.unit_quantity.label')"
                        ...
                        :errors="$errors->get('variant.unit_quantity')" for="unit_quantity">
                    ...
                    <x-hub::input.group :label="__(
                        ...
                        <x-hub::input.price wire:model="basePrices.{{ $this->currency->code }}.price"
                            :error="$errors->first('basePrices.'.$this->currency->code.'.price')"
                            :currencyCode="$this->currency->code" required />
                    </x-hub::input.group>
                    ...
                    <x-hub::input.group :label="__(
                        ...
                        <x-hub::input.price wire:model="basePrices.{{ $this->currency->code }}.compare_price"
            ...
            <div wire:key="group_price_{{ $group->id }}">
                <div class="grid items-center grid-cols-2 gap-4">
                    {{ $group->name }}
                    <x-hub::input.group :label="null" for="customerGroupPrices"
            ...
            <div class="flex items-center justify-between pt-4 border-t hidden">
                <div>
                ...
                <div class="relative flex divide-y divide-gray-100" wire:key="tier_{{ $index }}">
                    <div class="grid w-full grid-cols-6 gap-4 pr-8">
                        <div class="col-span-6 sm:col-span-6 lg:col-span-2">
                            <x-hub::input.group :label="__('adminhub::global.customer_group')"
                                for="customer_group_id_field_{{ $index }}"
                                :error="$errors->first('tieredPrices.' . $index . '.customer_group_id')" required>
                                <x-hub::input.select id="customer_group_id_field_{{ $index }}"
                        ...
                        <div class="col-span-6 sm:col-span-6 lg:col-span-2">
                            <x-hub::input.group :label="__('adminhub::global.lower_limit')"
                                for="tier_field_{{ $index }}"
                                :error="$errors->first('tieredPrices.' . $index . '.tier')" :error-icon="false"
                                required>
                                <x-hub::input.text id="tier_field_{{ $index }}"
                                    wire:model='tieredPrices.{{ $index }}.tier' type="number" min="2" steps="1" required
                        ...
                        <div class="col-span-6 sm:col-span-6 lg:col-span-2">
                            <x-hub::input.group :label="__(
                                ...
                                :error-icon="false" required>
                                <x-hub::input.price id="price_field_{{ $index }}_lang_{{ $currency->code }}"
