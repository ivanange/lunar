<header class="flex items-center justify-between">
  ...
  <button
    ...
    type="button" wire:click.prevent="$set('{{ $editTrigger }}', true)">
...
<address class="mt-4 text-sm not-italic text-gray-600">
  ...
  <div class="mt-2">
    <div class="flex items-center">
      ...
      <a href="tel:{{ $address->contact_phone }}" class="text-primary-500 underline">{{ $address->contact_phone }}</a>
      @else
    ...
    <div class="flex items-center">
      ...
      <a href="mailto:{{ $address->contact_email }}" class="text-primary-500 underline">{{ $address->contact_email
        }}</a>
      @else
      <span class="text-xs text-gray-500">{{ __('adminhub::global.not_provided') }}</span>
      @endif
