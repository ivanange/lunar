    ...
    <div class="overflow-hidden shadow sm:rounded-md">
        <div class="flex-col px-4 py-5 space-y-4 bg-white sm:p-6">
            <x-hub::input.group :label="__('adminhub::inputs.name')" for="name" :error="$errors->first('channel.name')"
                required>
                <x-hub::input.text wire:model="channel.name" name="name" id="name"
                    :error="$errors->first('channel.name')" />
            </x-hub::input.group>

            <x-hub::input.group :label="__('adminhub::inputs.handle')" for="handle"
                :error="$errors->first('channel.handle')" required>
                <x-hub::input.text wire:model.debounce.350ms="channel.handle" name="handle" id="handle"
                    :error="$errors->first('channel.handle')" />
            </x-hub::input.group>

            <x-hub::input.group :label="__('adminhub::inputs.url')" for="url" :error="$errors->first('channel.url')">
                <x-hub::input.text wire:model="channel.url" name="url" id="url"
                    :error="$errors->first('channel.url')" />
            </x-hub::input.group>

            <x-hub::input.group :label="__('adminhub::inputs.default.label')" for="handle"
                :instructions="__('adminhub::inputs.default.instructions', ['model' => 'channel'])">
                <x-hub::input.toggle wire:model="channel.default"
                    :disabled="$channel->id && $channel->getOriginal('default')" value="1" />
            </x-hub::input.group>
        </div>
