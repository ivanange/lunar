<div class="overflow-hidden shadow sm:rounded-md">
    <div class="flex-col px-4 py-5 space-y-4 bg-white sm:p-6">
        <div class="grid grid-cols-2 gap-4">
            <x-hub::input.group label="{{ __('adminhub::inputs.firstname') }}" for="firstname"
                :error="$errors->first('staff.firstname')">
                <x-hub::input.text wire:model="staff.firstname" name="firstname" id="firstname"
                    :error="$errors->first('staff.firstname')" />
            </x-hub::input.group>
            <x-hub::input.group label="{{ __('adminhub::inputs.lastname') }}" for="lastname"
                :error="$errors->first('staff.lastname')">
                <x-hub::input.text wire:model="staff.lastname" name="lastname" id="lastname"
                    :error="$errors->first('staff.lastname')" />
            </x-hub::input.group>
        </div>

        <x-hub::input.group label="{{ __('adminhub::inputs.email') }}" for="email"
            :error="$errors->first('staff.email')">
            <x-hub::input.text wire:model="staff.email" type="email" name="email" id="email"
                :error="$errors->first('staff.email')" />
        </x-hub::input.group>

        <div class="grid grid-cols-2 gap-4">
            <x-hub::input.group label="{{ __('adminhub::inputs.new_password') }}" for="password"
                :error="$errors->first('password')">
                <x-hub::input.text wire:model="password" type="password" name="password" id="password"
                    :error="$errors->first('password')" />
            </x-hub::input.group>
            <x-hub::input.group label="{{ __('adminhub::inputs.new_password_confirmation') }}"
                ...
                <x-hub::input.text wire:model="password_confirmation" type="password" name="password_confirmation"
                    id="passwordConfirmation" :error="$errors->first('passwordConfirmation')" />
            </x-hub::input.group>
        </div>
    </div>
</div>
...
@if ($staff->id)
<div @class([ 'bg-white border rounded shadow' , 'border-red-300'=> !$staff->deleted_at,
    'border-gray-300' => $staff->deleted_at,
    ])
    >
    <header @class([ 'px-6 py-4 bg-white border-b rounded-t' , 'border-red-300 text-red-700'=> !$staff->deleted_at,
...
<x-hub::modal.dialog wire:model="showRestoreConfirm">
    ...
    <x-slot name="footer">
        <div class="flex items-center justify-end space-x-4">
            ...
            <x-hub::button wire:click="restore" type="button" theme="green">
