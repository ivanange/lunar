    ...
    <div class="overflow-hidden shadow sm:rounded-md">
        <div class="flex-col px-4 py-5 space-y-4 bg-white sm:p-6">
            <div class="space-y-4 md:space-y-0 md:grid md:grid-cols-2 md:gap-4">
                <x-hub::input.group :label="__('adminhub::inputs.name')" for="name"
                    ...
                    <x-hub::input.text wire:model="language.name" name="name" id="name"
                        :error="$errors->first('language.name')" />
                </x-hub::input.group>
                <x-hub::input.group :label="__('adminhub::inputs.code')" for="code"
                    :error="$errors->first('language.code')" required>
                    <x-hub::input.text wire:model="language.code" name="code" id="code"
                        :error="$errors->first('language.code')" />
                </x-hub::input.group>
            </div>
            <x-hub::input.group label="Default" for="handle"
                :instructions="__('adminhub::settings.languages.form.default_instructions')">
                <x-hub::input.toggle wire:click="toggleDefault" :on="$language->default" name="handle" id="handle"
        ...
        <div class="px-4 py-3 text-right bg-gray-50 sm:px-6">
            <button type="submit"
                ...
                {{ __(
                $language->id ? 'adminhub::settings.languages.form.update_btn' :
                'adminhub::settings.languages.form.create_btn',
                ) }}
            </button>
        </div>
    </div>
            ...
            <div class="grid grid-cols-12 gap-4">
                <div class="col-span-12 md:col-span-6">
                    ...
                    <p class="text-xs text-gray-600">
                        {{ __('adminhub::inputs.danger_zone.instructions', ['attribute' => 'name', 'model' =>
                        'language']) }}
                    </p>
                </div>
                ...
                <div class="col-span-3 text-right lg:col-span-2">
                    <x-hub::button :disabled="!$this->canDelete" wire:click="delete" type="button" theme="danger">{{
