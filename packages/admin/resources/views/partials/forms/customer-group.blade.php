    ...
    <div class="overflow-hidden shadow sm:rounded-md">
        <div class="flex-col px-4 py-5 space-y-4 bg-white sm:p-6">
            <x-hub::input.group :label="__('adminhub::inputs.name')" for="name"
                ...
                <x-hub::input.text wire:model="customerGroup.name" :error="$errors->first('customerGroup.name')"
                    :placeholder="__('adminhub::components.attribute-edit.name.placeholder')"
                    wire:change="formatHandle" />
            </x-hub::input.group>

            <x-hub::input.group :label="__('adminhub::inputs.handle')" for="handle"
                :error="$errors->first('customerGroup.handle')">
                <x-hub::input.text wire:model.debounce.350ms="customerGroup.handle" name="handle" id="handle"
                    :error="$errors->first('customerGroup.handle')" />
            </x-hub::input.group>

            <x-hub::input.group label="Default" for="handle"
                :instructions="__('adminhub::settings.customer-groups.form.default_instructions')">
                <x-hub::input.toggle wire:model="customerGroup.default" :on="$customerGroup->default" name="handle"
                    id="handle" :disabled="$customerGroup->id && $customerGroup->getOriginal('default')" />
            </x-hub::input.group>
        </div>
        ...
        <div class="p-6 text-sm">
            <div class="grid grid-cols-12 gap-4">
                ...
                <div class="col-span-3 text-right lg:col-span-2">
                    <x-hub::button :disabled="!$this->canDelete" theme="danger" wire:click="delete" type="button">{{
