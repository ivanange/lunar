...
<div class="pb-24 mt-8 lg:gap-8 lg:flex lg:items-start">
    <div class="space-y-6 lg:flex-1">
        <div class="space-y-6">
            ...
            @if ($this->getVariantsCount() <= 1) <div id="pricing">
        ...
        @if ($product->id)
        <div @class([ 'bg-white border rounded shadow' , 'border-red-300'=> !$product->deleted_at,
...
<x-hub::layout.page-menu>
    <nav class="space-y-2" aria-label="Sidebar" x-data="{ activeAnchorLink: '' }"
        ...
        @foreach ($this->getSlotsByPosition('top') as $slot)
        <a href="#{{ $slot->handle }}" @class([ 'flex items-center gap-2 p-2 rounded text-gray-500'
            , 'hover:bg-primary-50 hover:text-primary-700'=> empty(
        ...
        @unless (in_array($item['id'], ['availability', 'associations']))
